...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException, ServletException {
        ...
        String access_token = JWT.create().withSubject(username). // identity the jwt to the user's username
                withExpiresAt(new Date(System.currentTimeMillis() + 10 * 60 * 1000))
                // URL of application
                .withIssuer(request.getRequestURL().toString())
                // then passes the permissions/authorities/roles for the user to the token
                .withClaim("roles", loggedInUser.getAuthorities().stream().map(GrantedAuthority::getAuthority)
                        .collect(Collectors.toList()))
                // sign token
                .sign(algorithm);

        String refresh_token = JWT.create().withSubject(username).
